...
def sync_database():
    """Drop all tables, recreate from models, and seed demo data."""
    ...
    # Create app with production config so it picks up DATABASE_URL
    ...
    with app.app_context():
        ...
        # Step 2: Create all tables from current models
        print("\n2. Creating tables from current models...")
        db.create_all()
        ...
        db.session.flush()
        print(f"   Users: admin (ID={admin_user.user_id}), tech1, tech2")
        ...
        print("   Memberships: admin=owner, tech1=technician, tech2=technician")
        ...
        print("   Subscription: starter (active)")
        ...
        for cust_idx, days_ago, completed, paid in jobs_data:
            j = Job(
                tenant_id=tenant.tenant_id,
                customer=customers[cust_idx].customer_id,
        ...
        for i, part in enumerate(parts):
            ...
            db.session.add(inv)
        db.session.flush()
        print(f"   Inventory: {len(parts)} records")
